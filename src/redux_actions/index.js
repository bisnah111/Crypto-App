...
export const fetchGlobal = () => async (dispatch, getState) => {
  const response1 = await coinGecko.get('/global')
  const response2 = await coinGecko.get('/global/decentralized_finance_defi')
  dispatch({
    type: GET_GLOBAL,
    payload: { ...response1.data.data, ...response2.data.data }
  })
}
